param(
      [string]$Installs = "",
      [ValidateSet("y","n","Y","N")][string]$Interactive = "n",
      [ValidateSet("y","n","Y","N")][string]$Archive4Doc = "y",
      [switch]$InstallFlux,
...
"@
    ...
    $actions = @()
    if ($Installs) {
      $actions = $Installs.Split(",") | ForEach-Object { $_.Trim().ToLowerInvariant() }
    }
    $doCygwin = $actions -contains "emit-cygwin-bootstrap"
    $doWSL    = $actions -contains "emit-wsl-setup"
    $doArtifacts = ($actions -contains "emit-wsl-cywin-artifacts") -or ($actions -contains "emit-wsl-cygwin-artifacts")
    ...
    if ($doCygwin) {
      ...
      $cygBash = Join-Path $cygRoot "vezde\.portable.bashrc"
      Write-StringToFileUtf8 -Path $cygBash -Body $PORTABLE_BASHRC_BODY
      $report.Add("emit: $cygBash")

      if ($doArtifacts) {
        Write-CheckNewLinuxArtifacts -DestRoots @($cygRoot)
        $report.Add("emit: linux_baseline under $cygRoot")
      }

      if ($Archive4Doc -match '^(y|Y)$') {
        $archDir = Join-Path $baseDir (".4doc_Cygwin_" + $sharedTS)
        New-DocArchiveCopy -SourceFile $cygBash -ArchiveDir $archDir -SharedTS $sharedTS
        $lb = Join-Path $cygRoot "linux_baseline"
        if (Test-Path $lb) {
          Get-ChildItem -LiteralPath $lb -File | ForEach-Object {
            New-DocArchiveCopy -SourceFile $_.FullName -ArchiveDir $archDir -SharedTS $sharedTS
          }
        }
      }
    }
    ...
    if ($doWSL) {
      ...
      $wslBash = Join-Path $wslRoot "vezde\.portable.bashrc"
      Write-StringToFileUtf8 -Path $wslBash -Body $PORTABLE_BASHRC_BODY
      $report.Add("emit: $wslBash")

      if ($doArtifacts) {
        Write-CheckNewLinuxArtifacts -DestRoots @($wslRoot)
        $report.Add("emit: linux_baseline under $wslRoot")
      }

      if ($Archive4Doc -match '^(y|Y)$') {
        $archDir = Join-Path $baseDir (".4doc_WSL_" + $sharedTS)
        New-DocArchiveCopy -SourceFile $wslBash -ArchiveDir $archDir -SharedTS $sharedTS
    ...
    Write-StringToFileUtf8 -Path $ppath -Body $PORTABLE_PROFILE_BODY
    $report.Add("emit: $ppath (patched helpers included)")
    ...
    Write-Host "Done. See report: $log"